...
var v2GetEventDetailsCmd = &cobra.Command{
	Use:   "get-event-details",
	Short: "Get Event Details",
	RunE:  withTelemetry(runV2GetEventDetails),
}

var v2GetEventDetailsFlags struct {
	id                     string
	locale                 string
...
func init() {
	v2GetEventDetailsCmd.Flags().StringVar(&v2GetEventDetailsFlags.id, "id", "", "ID of the event")
	v2GetEventDetailsCmd.MarkFlagRequired("id")
	v2GetEventDetailsCmd.Flags().StringVar(&v2GetEventDetailsFlags.locale, "locale", "", "The locale in ISO code format. Multiple comma-separated values can be provided. When omitting the country part of the code (e.g. only 'en' or 'fr') then the first matching locale is used. When using a '*' it matches all locales. '*' can only be used at the end (e.g. 'en-us,en,*') ")
	v2GetEventDetailsCmd.Flags().StringVar(&v2GetEventDetailsFlags.includeLicensedContent, "include-licensed-content", "", "True if you want to display licensed content")

	v2Cmd.AddCommand(v2GetEventDetailsCmd)
}

func runV2GetEventDetails(cmd *cobra.Command, args []string) error {
	...
	pathParams["id"] = fmt.Sprintf("%v", v2GetEventDetailsFlags.id)

	req := &httpclient.Request{
		Method:      "GET",
		Path:        httpclient.SubstitutePath("/discovery/v2/events/{id}", pathParams),
	...
	if cmd.Flags().Changed("locale") {
		req.QueryParams["locale"] = fmt.Sprintf("%v", v2GetEventDetailsFlags.locale)
	}
	if cmd.Flags().Changed("include-licensed-content") {
		req.QueryParams["includeLicensedContent"] = fmt.Sprintf("%v", v2GetEventDetailsFlags.includeLicensedContent)
	}
