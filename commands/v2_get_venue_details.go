...
var v2GetVenueDetailsCmd = &cobra.Command{
	Use:   "get-venue-details",
	Short: "Get Venue Details",
	RunE:  withTelemetry(runV2GetVenueDetails),
}

var v2GetVenueDetailsFlags struct {
	id                     string
	locale                 string
...
func init() {
	v2GetVenueDetailsCmd.Flags().StringVar(&v2GetVenueDetailsFlags.id, "id", "", "ID of the venue")
	v2GetVenueDetailsCmd.MarkFlagRequired("id")
	v2GetVenueDetailsCmd.Flags().StringVar(&v2GetVenueDetailsFlags.locale, "locale", "", "The locale in ISO code format. Multiple comma-separated values can be provided. When omitting the country part of the code (e.g. only 'en' or 'fr') then the first matching locale is used. When using a '*' it matches all locales. '*' can only be used at the end (e.g. 'en-us,en,*') ")
	v2GetVenueDetailsCmd.Flags().StringVar(&v2GetVenueDetailsFlags.includeLicensedContent, "include-licensed-content", "", "True if you want to display licensed content")

	v2Cmd.AddCommand(v2GetVenueDetailsCmd)
}

func runV2GetVenueDetails(cmd *cobra.Command, args []string) error {
	...
	pathParams["id"] = fmt.Sprintf("%v", v2GetVenueDetailsFlags.id)

	req := &httpclient.Request{
		Method:      "GET",
		Path:        httpclient.SubstitutePath("/discovery/v2/venues/{id}", pathParams),
	...
	if cmd.Flags().Changed("locale") {
		req.QueryParams["locale"] = fmt.Sprintf("%v", v2GetVenueDetailsFlags.locale)
	}
	if cmd.Flags().Changed("include-licensed-content") {
		req.QueryParams["includeLicensedContent"] = fmt.Sprintf("%v", v2GetVenueDetailsFlags.includeLicensedContent)
	}
